    ...
    React.useEffect(() => {
      ...
      if (routePoints.length > 1) {
        const route = L.polyline(routePoints, {
          color: '#c2693a', weight: 1.8, opacity: 0.55,
      ...
      ROUTE_CHAPTERS.forEach((c) => {
        ...
        const num = c.kind === 'chapter' ? (c.num || '').toString() : '·';
        const html = `
        ...
        m.addTo(map);
        layersRef.current.chapters.push({ id: c.id, marker: m });
      });
    ...
    React.useEffect(() => {
      ...
      const doFit = () => {
        ...
        if (placeCoords.length > 1) {
          map.fitBounds(placeCoords, { padding: [40, 40], maxZoom: 9, animate: false });
        } else if (placeCoords.length === 1) {
          map.setView(placeCoords[0], 8, { animate: false });
        } else if (ch.anchor) {
          map.setView(ch.anchor, 7, { animate: false });
        }
      };
      setTimeout(doFit, 50);
