...
(function () {
  ...
  function GlobeView({
    ...
    function pinEl(c) {
      ...
      wrap._id = c.id;
      wrap.innerHTML = `<div class="map-pin" data-chapter="${c.id}">` + `<div class="map-pin-num" style="--pin-c: ${region.accent};">${c.num == null ? '' : c.num}</div>` + `<div class="map-pin-label">${c.title}</div></div>`;
    ...
    function refreshSelection() {
      ...
      ch.places.forEach((p, i) => {
        ...
        wrap.innerHTML = `<div class="map-place${i === selectedPlaceIdx ? ' is-active' : ''}">` + `<div class="map-place-dot" style="--pin-c: ${region.accent}"></div>` + `<div class="map-place-label">${i + 1}. ${p.name}</div></div>`;
