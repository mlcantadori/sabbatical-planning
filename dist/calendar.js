...
(function () {
  ...
  async function findOrCreateCalendar() {
    ...
    if (hit) {
      try {
        localStorage.setItem(LS_CAL, hit.id);
      } catch {}
      return hit.id;
    }
    const created = await window.gapi.client.calendar.calendars.insert({
      resource: {
        summary: CAL_NAME,
        description: 'Asia sabbatical 2026–2027 — synced from the trip planner',
        timeZone: 'America/Sao_Paulo'
      }
    });
    try {
      localStorage.setItem(LS_CAL, created.result.id);
    } catch {}
  ...
  async function upsert(calendarId, ev) {
    ...
    try {
      ...
      return {
        ...
        id: ev.id,
        summary: ev.summary,
        error: errText(e2) + ' [patch: ' + errText(patchErr) + ']'
      };
    }
  }
  async function prune(calendarId, wantedIds) {
    ...
    const stale = (res.result.items || []).filter(it => it.id && it.id.indexOf('sabbatical') === 0 && wantedIds.indexOf(it.id) < 0);
  ...
  async function syncAll(onStatus) {
    ...
    try {
      ...
      localStorage.setItem('gcal-last-report', JSON.stringify({
        at: stamp,
        synced: n,
        failed
      }));
  ...
  function SyncButton({
    ...
    return /*#__PURE__*/React.createElement("button", {
      onClick: run,
      title: "Push chapter dates + flight blocks to your Google Calendar"
    }, /*#__PURE__*/React.createElement(window.Icon.calendar, {
      size: 12
    }), " ", !compact && label);
  }
