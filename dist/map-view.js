...
(function () {
  ...
  function MapView({
    ...
    React.useEffect(() => {
      ...
      const svg = d3.select(el).append('svg').attr('class', 'eq-svg').attr('width', '100%').attr('height', '100%');
      ...
      const zoom = d3.zoom().scaleExtent([1, MAX_ZOOM]).translateExtent([[-2000, -2000], [4000, 4000]]).on('start', () => {
        st.current.userMoved = true;
      }).on('zoom', e => {
      ...
      ro.observe(el);
      fetch('world-110m.json').then(r => r.json()).then(world => {
    ...
    function size(refit) {
      ...
      if (refit && !s.userMoved && s.world) {
        s.projection.fitExtent([[0, 0], [w, h]], {
          type: 'Sphere'
        });
    ...
    function drawWorld() {
      ...
      s.g.select('.eq-sphere').attr('d', s.path({
        type: 'Sphere'
      }));
      s.g.select('.eq-graticule').attr('d', s.path(window.d3.geoGraticule10()));
      s.g.select('.eq-land').datum(countries).attr('d', s.path);
      s.g.select('.eq-borders').datum(window.topojson.mesh(s.world, s.world.objects.countries, (a, b) => a !== b)).attr('d', s.path);
      drawRoute();
    }
    ...
    function drawRoute() {
      const s = st.current;
      if (!s) return;
      const pts = propsRef.current.chapters.filter(c => c.kind !== 'transit' && c.anchor).map(c => ll(c.anchor));
      if (pts.length > 1) {
        s.routeSel.datum({
          type: 'LineString',
          coordinates: pts
        }).attr('d', s.path).style('display', null);
      } else {
    ...
    function fullRouteBox(padLeft) {
      ...
      return {
        bbox: b,
        extent: [[left, pad], [s.w - pad, s.h - pad]]
      };
    }
